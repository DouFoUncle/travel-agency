 ...
 * @param time
 */
function message(str, icon, time, callback) {
    if (!icon) {
        layer.msg(str, {time: time*1000 || 1000, shade: 0.3, end: !callback ? () => {} : callback});
    } else{
        layer.msg(str, {time: time*1000 || 1000, shade: 0.3, icon: icon, end: !callback ? () => {} : callback});
    }
}
...
function sendJsonObjRequest(method, url, jsonObj, isRefreshParent) {
    $.ajax({
        ...
        success: function(rollData) {
            if (rollData.code == "200" || rollData.code == "0") {
...
function sendUrlRequest(method, url, isRefreshParent) {
    $.ajax({
        ...
        async: false,
        success: function(rollData) {
            if (rollData.code == "0" || rollData.code == "200") {
                layer.confirm(rollData.msg, {
                    btn: ['确定']  //按钮
                    , icon: 6
                }, function () {
 ...
 * 提示窗口
 */
function tips(isPrice) {
    let content = '如需修改直接单击要修改的数据的单元格就可开始修改哦！';
    if(isPrice === '1') {
        content += '<span style="color: #FF5722;">修改价格时需要注意输入的单位为分而不是元！</span>';
    }
    layer.open({
        title: '消息提示'
        , content: content
 ...
 * @param text
 */
function tipsMini(text, domId) {
    layer.tips(text, domId, {
        tips: [2, '#3595CC'],
        time: 1000
    });
}
