...
<script id="operation" type="text/html">
    <a class="layui-btn layui-btn-xs layui-btn-normal" style="height: 26px;line-height: 28px;font-size: 13px; padding: 0 10px;" lay-event="find">查看</a>
...
<script>
    layui.use(['table', 'form', 'laydate'],
        function () {
            ...
            // 初始调用表格加载方法
            loadFunction(getPath() + "/web/member/collect/getPageList");
            ...
            $("#searchBtn").click(function () {
                ...
                // 重新调用表格加载方法
                loadFunction(getPath() + "/web/member/collect/getPageList?" + sendData)
            })
                    ...
                    , cols:
                        [
                            [ //表头
                                ...
                                , {field: 'strategyId', title: 'id', hide: true}
                                , {field: 'title', title: '收藏的攻略', width: '28.6%', templet: '<div>{{d.strategy.title}}</div>'}
                                , {field: 'createDate', title: '收藏时间', width: '28.6%'}
            ...
            table.on('tool(listInfo)', function(obj){
                ...
                switch(obj.event){
                    // 查看攻略
                    case 'find':
                        top.open("<%= ServletUtils.getProjectHttpUrl(request)%>web/system/toWebStrategyDetailPage?id=" + obj.data.strategyId, "_blank");
                        break;
                    // 删除
                    case 'del':
                        // 参数分别表示：选中的数据， 请求方式， 请求地址， 提示消息（默认提示删除）， 提示显示的icon（默认显示删除）， 是否使用JSON对象传递参数
                        promptRequest(1, "GET", getPath() + "/web/member/collect/deleteInfo?ids=" + id, null, null, false);
                        break;
                }
            });
        });
