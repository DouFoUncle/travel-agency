...
<script>
    layui.use(['table', 'form', 'laydate'],
        function () {
            ...
            // 初始调用表格加载方法
            loadFunction(getPath() + "/web/member/comment/getPageList");
            ...
            $("#searchBtn").click(function () {
                ...
                // 重新调用表格加载方法
                loadFunction(getPath() + "/web/member/comment/getPageList?" + sendData)
            })
                    ...
                    , cols:
                        [
                            [ //表头
                                ...
                                , {field: 'title', title: '评价的攻略', width: '20.6%', templet: '<div>{{d.strategy.title}}</div>'}
                                , {field: 'content', title: '评价内容', width: '20.6%'}
                                , {field: 'createDate', title: '浏览时间', width: '20.6%'}
                                , {field: '操作', title: '操作', templet: '#operation', width: '21.8%'}
                            ]
                        ]
            ...
            table.on('tool(listInfo)', function(obj){
                ...
                switch(obj.event){
                    ...
                    case 'del':
                        ...
                        promptRequest(1, "GET", getPath() + "/web/member/comment/deleteInfo?ids=" + id, null, null, false);
                        break;
                }
            });
        });
