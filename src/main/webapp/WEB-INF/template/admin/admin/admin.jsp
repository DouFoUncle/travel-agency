...
<script>
    layui.use(['table', 'form', 'laydate'],
        function () {
            ...
            // 初始调用表格加载方法
            loadFunction("<%= ServletUtils.getProjectHttpUrl(request)%>admin/getPageList");
            ...
            $("#searchBtn").click(function () {
                ...
                // 重新调用表格加载方法
                loadFunction("<%= ServletUtils.getProjectHttpUrl(request)%>admin/getPageList?" + sendData)
            })
                    ...
                    , cols:
                        [
                            [ //表头
                                ...
                                , {field: 'userName', title: '用户名', width: '29%'}
                                , {field: 'loginName', title: '用户昵称', width: '29.5%'}
                                , {field: 'createDate', title: '注册时间', width: '29.5%'}
                            ]
                        ]
            ...
            table.on('toolbar(listInfo)', function(obj){
                ...
                switch(obj.event){
                    ...
                    case 'add':
                        // 跳转到新增页面
                        toWindow("<%= ServletUtils.getProjectHttpUrl(request)%>/admin/toAddWindow", "新增信息", "430px", "330px", true);
                        break;
                    // 响应删除按钮
                    case 'del':
                        // 参数分别表示：选中的数据， 请求方式， 请求地址， 提示消息（默认提示删除）， 提示显示的icon（默认显示删除）， 是否使用JSON对象传递参数
                        promptRequest(selectCount, "GET", "<%= ServletUtils.getProjectHttpUrl(request)%>admin/deleteInfo?ids=" + dataId, null, null, false);
                        break;
                }
            });
        });
