                    ...
                    <form class="layui-form layui-col-space5 layui-form-pane" id="form">
                        ...
                        <div class="layui-inline layui-show-xs-block layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" class="layui-input" name="title" placeholder="请输入">
                            </div>
                        </div>
                        ...
                        <div class="layui-inline layui-show-xs-block layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <select name="userType" id="userType" class="layui-select">
                                    <option value="">请选择</option>
                                    <option value="1">管理员发布</option>
...
<script id="operation" type="text/html">
    ...
    {{# if(d.userId == ${sessionScope.adminUser.id} && d.userType == '1') {}}
...
<script>
    layui.use(['table', 'form', 'laydate'],
        function () {
            ...
            // 初始调用表格加载方法
            loadFunction(getPath() + "/strategy/getPageList");
            ...
            $("#searchBtn").click(function () {
                ...
                // 重新调用表格加载方法
                loadFunction(getPath() + "/strategy/getPageList?" + sendData)
            })
                    ...
                    , cols:
                        [
                            [ //表头
                                ...
                                , {field: 'title', title: '攻略标题', width: '14.8%'}
                                , {field: 'resume', title: '攻略简述', width: '15%'}
                                , {field: 'createDate', title: '发布时间', width: '14%'}
                                , {field: '操作', title: '操作', templet: '#operation', width: '14%'}
                            ]
                        ]
            ...
            table.on('tool(listInfo)', function(obj){
                ...
                switch(obj.event){
                    ...
                    case 'find':
                        // 打开查看详情弹窗
                        toWindow(getPath() + "/strategy/toFindWindow?id=" + id, "查看信息", "730px", "600px", true);
                        break;
                    // 修改
                    case 'edit':
                        // 打开修改弹窗
                        toWindow(getPath() + "/strategy/toEditWindow?id=" + id, "修改信息", "730px", "600px", true);
                        break;
                }
            });
            ...
            table.on('toolbar(listInfo)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        // 跳转到新增页面
                        toWindow(getPath() + "/strategy/toAddWindow", "新增信息", "730px", "650px", true);
                        break;
                    // 删除
                    case 'del':
                        // 参数分别表示：选中的数据， 请求方式， 请求地址， 提示消息（默认提示删除）， 提示显示的icon（默认显示删除）， 是否使用JSON对象传递参数
                        promptRequest(selectCount, "GET", getPath() + "/strategy/deleteInfo?ids=" + dataId, null, null, false);
                        break;
                }
            });
        });
