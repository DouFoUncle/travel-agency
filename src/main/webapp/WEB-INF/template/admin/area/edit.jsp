...
<div style="padding: 15px;">
    <form id="myform" action="" method="post" class="layui-form-pane layui-form">
                ...
                <select name="provinceId">
                    ...
                    <c:forEach items="${areaList}" var="item">
                        <option value="${item.id}"
                                data-value="${obj.provinceId}"
                                <c:if test="${obj.provinceId == item.id}">selected</c:if>>${item.areaName}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        ...
        <div class="layui-inline layui-show-xs-block layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" class="layui-input" name="areaName" placeholder="请输入" value="${obj.areaName}">
            </div>
        </div>

        <div style="position: absolute; bottom: 15px; left: 15px">
            <input type="hidden" class="layui-input" name="parent" value="${obj.isParent}">
            <input type="hidden" class="layui-input" name="id" value="${obj.id}">
